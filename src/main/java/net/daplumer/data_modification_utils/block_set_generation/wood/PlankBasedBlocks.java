...
public final class PlankBasedBlocks {
    ...
    public void registerItemTags(FabricTagProvider.ItemTagProvider tagProvider){
        final FabricTagProviderAccessor accessor = (FabricTagProviderAccessor) tagProvider;
         add(accessor.builder (ItemTags.PLANKS), planksItem);
         add(accessor.builder (ItemTags.WOODEN_FENCES), fenceItem);
         add(accessor.builder (ItemTags.WOODEN_PRESSURE_PLATES), pressurePlateItem);
         add(accessor.builder (ItemTags.WOODEN_BUTTONS), buttonItem);
         add(accessor.builder (ItemTags.WOODEN_SLABS), slabItem);
         add(accessor.builder (ItemTags.WOODEN_STAIRS), stairsItem);
         add(accessor.builder (ItemTags.FENCE_GATES), fenceGateItem);
        if (!planks.getDefaultState().isBurnable()) {
                add(accessor.builder(ItemTags.NON_FLAMMABLE_WOOD), planksItem, fenceItem, pressurePlateItem, buttonItem, slabItem, stairsItem, fenceGateItem);}
    }
    public void registerBlockTags(FabricTagProvider.BlockTagProvider tagProvider){
        final FabricTagProviderAccessor accessor = (FabricTagProviderAccessor) tagProvider;
         add(accessor.builder (BlockTags.PLANKS), planks);
         add(accessor.builder (BlockTags.FENCE_GATES), fenceGate);
         add(accessor.builder (BlockTags.WOODEN_STAIRS), stairs);
         add(accessor.builder (BlockTags.WOODEN_SLABS), slab);
         add(accessor.builder (BlockTags.WOODEN_BUTTONS), button);
         add(accessor.builder (BlockTags.WOODEN_FENCES), fence);
         add(accessor.builder (BlockTags.WOODEN_PRESSURE_PLATES), pressurePlate);
    }
    private void add(ProvidedTagBuilder<RegistryKey<Item>,Item> builder, Item... instances){
        Arrays.stream(instances).forEach((item -> builder.add(RegistryKey.of(RegistryKeys.ITEM, Registries.ITEM.getId(item)))));
    }
    private void add(ProvidedTagBuilder<RegistryKey<Block>,Block> builder, Block... instances){
        Arrays.stream(instances).forEach((block -> builder.add(RegistryKey.of(RegistryKeys.BLOCK, Registries.BLOCK.getId(block)))));
    }
