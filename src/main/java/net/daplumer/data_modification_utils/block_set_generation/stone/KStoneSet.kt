...
class KStoneSet(
    ...
    private fun ProvidedTagBuilder<RegistryKey<Block>,Block>.add(vararg value:Block){
        value.forEach { this.add(RegistryKey.of(RegistryKeys.BLOCK, Registries.BLOCK.getId(it))) }
    }
    private fun ProvidedTagBuilder<RegistryKey<Item>,Item>.add(vararg value: Item){
        value.forEach { this.add(RegistryKey.of(RegistryKeys.ITEM, Registries.ITEM.getId(it))) }
    }
    ...
    class BlockSubSet {
        ...
        val variant: Variant
        val name: String
        val crackedBlock:Block?
        val chiseledBlock:Block?
        val stairsBlock:StairsBlock
        val slabBlock:SlabBlock
        val wallBlock:WallBlock

        constructor(defaultBlock:Block,variant: Variant,name: String, registerer: GeneralDataRegisterer<Block, Settings>, hasCrackedVariant: Boolean = false, hasChiseledVariant: Boolean = false) {
            this.defaultBlock = defaultBlock
            this.variant = variant
            this.name = name
            crackedBlock = if (hasCrackedVariant)
                registerer.register(
                    "cracked_" + variant.createName(name, true),
                    copyShallow(defaultBlock)
                ) else null
            chiseledBlock = if (hasChiseledVariant)
                registerer.register(
                    "chiseled_"+ variant.createName(name, true),
                    copyShallow(defaultBlock)
                ) else null

            stairsBlock = registerer.register(
                variant.createName(name, false) + "_stairs",
                copyShallow(defaultBlock),
                STAIRS(defaultBlock.defaultState)
            )
            slabBlock = registerer.register(
                variant.createName(name, false) + "_slab",
                copyShallow(defaultBlock)
            ) { SlabBlock(it) }

            wallBlock = registerer.register(
        ...
        //fun registerItemTags(builder: (tag:TagKey<Item>) ->ProvidedTagBuilder<RegistryKey<Item>, Item>) {
        //    builder(ItemTags.SLABS).add(slabBlock.asItem())
        //    builder(ItemTags.STAIRS).add(stairsBlock.asItem())
        //    builder(ItemTags.WALLS).add(wallBlock.asItem())
        //}

        fun registerBlockTags(builder: (tag:TagKey<Block>) ->TagBuilder) {
            builder(BlockTags.PICKAXE_MINEABLE).add(defaultBlock, slabBlock, stairsBlock, wallBlock)
            builder(BlockTags.SLABS).add(slabBlock)
            builder(BlockTags.STAIRS).add(stairsBlock)
            builder(BlockTags.WALLS).add(wallBlock)
            crackedBlock?.let {  builder(BlockTags.PICKAXE_MINEABLE).add(it) }
            chiseledBlock?.let { builder(BlockTags.PICKAXE_MINEABLE).add(it) }
        }
        private fun TagBuilder.add(vararg value:Block){
            value.forEach {add(Registries.BLOCK.getId(it)) }
        }
        private fun TagBuilder.add(vararg value: Item){
            value.forEach {add(Registries.ITEM.getId(it)) }
        }
    ...
    enum class Variant(val prefix: String, val affix: String, val plural: Boolean) {
        ...
        MOSAIC("mosaic_","",false),
        SMOOTH("smooth_","",false);
    ...
    fun registerBlockTags(builder: (tag:TagKey<Block>) ->TagBuilder, mineabilityTag: TagKey<Block>?) {
        variants.values.forEach { smallBlockSet: BlockSubSet ->
            smallBlockSet.registerBlockTags(
                builder
            )
        }
        mineabilityTag?.let{ variants.values.forEach { set: BlockSubSet ->
                set.list().forEach { block: Block ->
                        builder(mineabilityTag).add(block)
                }
            }
        }
    }
    ...
    fun registerLootTables(lootTableProvider: FabricBlockLootTableProvider) {
        setsWithoutDefault.forEach { set: BlockSubSet ->
        ...
        if (variants.containsKey(Variant.DEFAULT)) {
            if (variants.containsKey(Variant.COBBLED)) {
    ...
    private inner class ItemTagProvider(output:FabricDataOutput, completableFuture: CompletableFuture<RegistryWrapper.WrapperLookup>):
        ...
        override fun getTagBuilder(tag: TagKey<Item>?): TagBuilder {
        ...
        override fun configure(wrapperLookup: RegistryWrapper.WrapperLookup?) {
            variants.values.forEach { smallBlockSet: BlockSubSet ->
                    getTagBuilder(ItemTags.SLABS).add(smallBlockSet.slabBlock.asItem())
    ...
    inner class BlockTagProvider(output:FabricDataOutput, completableFuture: CompletableFuture<RegistryWrapper.WrapperLookup>,private val mineabilityTag: TagKey<Block>):
        ...
        override fun configure(wrapperLookup: RegistryWrapper.WrapperLookup?) {
            registerBlockTags(this::getTagBuilder, mineabilityTag)
        }
    }
    ...
    fun provideLang(pack:FabricDataGenerator.Pack){
